...
<script>
	...
	function sub(){
		...
		if(null != addId){
			window.location="${base}/order/save?addressId="+addId+"&itemAllPrice=${itemAllPrice}";
		};
	}
...
function initProvince(){
		...
		xmlhttp.onreadystatechange =function(){
			if(xmlhttp.readyState==4&&xmlhttp.status==200){
				var provinceStr=xmlhttp.responseText;
				var province=JSON.parse(provinceStr);
				for(var i=0;i<province.length;i++){
					var option=document.createElement("option");
					option.value=province[i].provinceCode;
					option.text=province[i].provinceName;
					document.getElementById("province").options[i+1]=option;
				}
			}
		};
	...
	function initCity(){
		...
		var url = "${base}/ssxjl/city?province="+province;
		xmlhttp.open("GET", url, true);
		//3.设置请求头
		xmlhttp.send();
		...
		xmlhttp.onreadystatechange =function(){
			if(xmlhttp.readyState==4&&xmlhttp.status==200){
				...
				for(var i=0;i<city.length;i++){
					...
					option.text=city[i].cityName;
					document.getElementById("city").options[i+1]=option;
				}
			}
		};
	}
	...
	function initArea(){
		...
		var url = "${base}/ssxjl/area?city="+city;
		xmlhttp.open("GET", url, true);
		//3.设置请求头
		xmlhttp.send();
		...
		<div class="center_content">
			<div class="left_content">
				...
				<div class="address_more">
					...
					<a href="${base }/permission/order/balance?pageSize=${addressPageing.pageSize+3}">更多地址</a>
				</div>
				...
				<div id="new_Address" class="new_Address">
					...
					<div class="newAddress_detail">
						<form action="${base }/address/add" method="post">
							<table>
								<tr>
									...
									</select> &nbsp; <select id="city" name="city" onchange="initArea()">
					...
					<table class="cart_table">
						...
						<c:if test="${!empty user }">
							<c:forEach var="item" items="${newItems }">
								<tr>
									<td><a
										href="${base }/book/detailBook?bookId=${item.book.bookId}"><img
				...
				<div class="right_box">
					...
					<ul class="list">
						<c:forEach items="${categorys }" var="category">
							<li><a
								href="${base }/book/viewBookByCate?cateId=${category.cateId}">${category.cateName
									}</a></li>
						</c:forEach>
					</ul>
