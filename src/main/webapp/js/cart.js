...
function removeCart() {
	...
	xmlHttp.open("get", "/bms/cart/removecart?ids="
			+ ids, true);
	xmlHttp.send();
...
function modifyItem(id, obj, vs) {
	...
	if (/^[1-9]\d*$/.test(count)) {
		...
		xmlHttp.open("get", "/bms/cart/modifyCart?bookId="
				+ id + "&count=" + count, true);
		xmlHttp.send();
...
function clearCart() {
	...
	xmlHttp.open("get", "/bms/cart/clearCart", true);
	xmlHttp.send();
...
function balanceItem() {
	...
	if(count > 0){
		window.location="/bms/permission/order/balance?ids="
